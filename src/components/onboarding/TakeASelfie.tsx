...
export const TakeASelfie: React.FC<OnboardingProps> = ({ advance, goBack }) => {
						...
						if (context) {
								...
								img.onload = () => {
										...
										if (!validation.brightness) {
												...
												toast.error("The image is too blurry. Please try again.");
												setCapturedImage(null);
										} else {
												toast.success("Image captured successfully! Looks great.");
										}
								};
						}
				}
		...
		function validateImageClarity(image: HTMLImageElement): { brightness: boolean; blur: boolean } {
				...
				for (let i = 0; i < data.length; i += 4) {
						const avg = (data[i] + data[i + 1] + data[i + 2]) / 3; // RGB average
						totalBrightness += avg;
				}
				...
				const blurOK = laplacianVariance > 100; // Adjust the threshold as needed
		...
		function calculateLaplacianVariance(_image: HTMLImageElement, canvas: HTMLCanvasElement, context: CanvasRenderingContext2D) {
				...
				for (let i = 0; i < imageData.data.length; i += 4) {
						gray[i / 4] = 0.2989 * imageData.data[i] + 0.5870 * imageData.data[i + 1] + 0.1140 * imageData.data[i + 2]; // Grayscale
				}
						...
						<section className="h-[500px] overflow-hidden">
								...
								<div className={`flex flex-col gap-y-[30px] h-full`}>
										...
										<div className={`grid gap-y-6`}>
												<div className={`w-[250px] h-[250px] rounded-[15px] bg-opacity-20 bg-[#8A8A8E] relative`}>
														<video className={`size-full absolute z-10`} ref={videoRef} autoPlay style={{ width: '100%' }}></video>
														...
														<button className={`absolute z-[60] bg-black/80 text-white px-3 py-2 rounded-[8px] bottom-[10px] right-[10px] font-bold text-[10px]`} onClick={captureImage}>Capture</button>
												</div>
