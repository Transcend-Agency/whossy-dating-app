...
export const ChatListItem: React.FC<ChatListItemProps> = ({profileImage, contactName, message, messageStatus, openChat, onlineStatus, chatInterface, userData}) => {
  return (
    <div className='flex justify-between cursor-pointer hover:bg-[#f9f8f8] px-[1.6rem] pb-[0.6rem] pt-[1.4rem] transition-all duration-300 ease-in-out transform hover:scale-[1.02]' style={{borderBottom: '1px solid #F6F6F6'}} onClick={userData?.is_premium ? openChat : () => toast.error('Upgrade plan to view message')}>
        <div className='flex gap-x-[0.8rem]'> 
            ...
            <div>
                ...
                {userData?.is_premium ? <p className={` text-[1.6rem] leading-[1.92rem] ${message === 'Image' ? 'italic text-[#b2b2b5]' : 'text-[#8A8A8E]'}`}>{ message !== "Image" ? chatInterface ? message.length > 25 ? message.slice(0,25) + '...' : message : message.length > 35 ? message.slice(0,35) + '...' : message : 'sent a photo'}</p> : <p className=" text-[1.6rem] leading-[1.92rem] text-[#8A8A8E] ">Upgrade plan to view this message</p>}
            </div>
        </div>
