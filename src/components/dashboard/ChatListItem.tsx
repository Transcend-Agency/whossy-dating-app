...
const ChatListItem: React.FC<ChatListItemProps> = ({profileImage, contactName, message, messageStatus, onlineStatus}) => {
  return (
    <div className='flex  justify-between cursor-pointer pb-[0.6rem]' style={{borderBottom: '1px solid #F6F6F6'}}>
       ...
       {messageStatus && <p className='bg-[#F6F6F6] text-[1.4rem] font-normal h-fit px-[0.6rem] rounded-[0.6rem]'>Unread</p>}
    </div>
  )
}
