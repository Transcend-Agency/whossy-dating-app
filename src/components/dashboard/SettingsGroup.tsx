        ...
        {data.map((item, index) => (
            <button key={index} onClick={item[2]} className="settings-page__settings-group__item">
                <div className="settings-page__settings-group__item-container">
                    ...
                    {item[1] !== undefined ? <div className="settings-page__settings-group__item__value mr-[1.3rem]">{item[1].length > 40 ? item[1].substring(0, 40) + '...' : item[1]}{(item[0] !== "Birthday" && item[0] !== "Email") && <img src="/assets/icons/arrow-right.svg" alt={``}/>}</div> : <div className='flex items-center gap-x-3 mr-[1.3rem]'>
