...
export const ImagesModal: React.FC<ActionsModalProps & {imageUrl: string}> = ({show, hide, chatName, blockUser, reportUser, imageUrl}) => {
  ...
  return (
    <DashboardSettingsModal showing={show} title="Send Image" hideModal={hide}>
         <div className="flex flex-col gap-y-4">
          ...
          /> <IoSend className='cursor-pointer'/> </div>
         </div>
     </DashboardSettingsModal>
  )
}